...
The reviewer closed N-1/N-3/N-4 and found no new freeze blocker beyond N-2/N-5. R-1 exposed that the historical `1e-8` per-IE mean check, valid for a fixed-parameter solver comparison, failed for 12 of 40 independently refitted cross-platform paths (reviewer-reported maximum `9.08e-8`). The old comparison remains visible but is explicitly non-acceptance; an independent development block must set the separate refitted tolerance before validation. The raw reproduction bundle has not been supplied to this checkout.
