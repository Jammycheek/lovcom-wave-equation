...
| F-10 | refitted-forecast equivalence diagnostic failure, reviewer-reported | The same Linux reproduction reportedly had maximum per-IE holdout-mean difference `9.08e-8`; `12/40` replicate pairs exceeded the historical `1e-8` fitted-implementation test. The value was borrowed from a fixed-parameter ODE solver check and applied to separately fitted parameters. The raw cross-platform bundle remains unavailable in this checkout. Keep the solver check; specify a distinct refitted-forecast equivalence tolerance on independent development data before validation. No exposed replicate is used for calibration. |
