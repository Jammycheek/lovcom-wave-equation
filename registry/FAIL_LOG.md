...
| F-07 | numerical implementation failure | The first non-truth-start benchmark reported `0/40` strict-converged fits because an invalid-ODE trial returned a flat `1e300` objective penalty. L-BFGS-B line-search interpolation collapsed and falsely reported success at non-stationary initial points with large gradients. The original `0/40` artifact is retained as an invalidated diagnostic result; the finite-gradient penalty and explicit projected-gradient gate correct the implementation without changing the RCWE model. Full corrected rerun pending. |
